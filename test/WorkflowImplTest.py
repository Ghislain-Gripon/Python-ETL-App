...
class WorkflowImplTest(unittest.TestCase):
    ...
    def test_clean_clinical_trials(self):
        ...
        val_pubmed_and_clinical_trials_df:DataFrame = DataFrame({
            "id" : [1, 2],
            "title": ["A 44-YEAR-OLD MAN WITH ERYTHEMA OF THE FACE DIPHENHYDRAMINE, NECK, AND CHEST, WEAKNESS, AND PALPITATIONS", "AN EVALUATION OF BENADRYL, PYRIBENZAMINE, AND OTHER SO-CALLED DIPHENHYDRAMINE ANTIHISTAMINIC DRUGS IN THE TREATMENT OF ALLERGY."],
    ...
    def test_get_files_by_type(self):
        ...
        file_dict:dict[str, list[dict[str, Path | str]]] = workflow._get_files_by_type(workflow.config["data_directory_path"]["data"]["file_type"])
    ...
    def test_build_data_frames(self):
        ...
        for file_type, value in file_dict.items():
            ...
            for entry in value:
                ...
                workflow.get_data_loader_class(entry["extension"]).as_dataframe(entry["file_path"])))
