...
class WorkflowImpl(Workflow):
    ...
    @debug
    def _get_files_by_type(self, file_types: list[dict]) -> list[dict[str, Path | str]]:
    ...
    def _clean_pubmed_or_clinical_trials(self, pubmed_or_clinical_trials_data: DataFrame, data_cleaner: DataCleaner) -> DataFrame:
        ...
        pubmed_or_clinical_trials_data_fill_missing_by_id[["title", "date", "journal"]] = \
            pubmed_or_clinical_trials_data_fill_missing_by_id.groupby("id").bfill()[["title", "date", "journal"]]
